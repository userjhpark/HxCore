...
namespace HxCore.Win.PDFXE
{
    partial class HxPDFXE
    {
        ...
        public int? GetStr2NumberID(string value)
        {
            try
            {
                return PDFCtl.Inst.Str2ID(value);
            }
            catch (Exception ex)
            {
        ...
        public string GetNumber2StrID(int? value, bool bReplaceChar = false)
        {
            try
            {
                if (!value.IsNullOrZeroMinEx())
                {
                    string Result = PDFCtl.Inst.ID2Str((int)value);
                    if(bReplaceChar == true)
                    {
                        Result = Result.RegexReplaceEx(@"\_", @"\.");
                    }
                    return Result;
                }
            }
        ...
        private int[] GetIDSArray()
        {
            ...
            for (HxIDS i = 0; i < HxIDS._last_; i++)
            {
                ...
                sid = sid.Replace('_', '.');
                Result[(int)i] = PDFCtl.Inst.Str2ID(sid);
            }
            return Result;
        }
        /// <summary>
        ...
        public int? GetIDS2NumberID(HxIDS value)
        {
            ...
            {
                return (int)PDFCtl.Inst.Str2ID(strID);
            }
            return null;
        }
        ...
        {
            PDFCtl.EnableEventListening2(nID, bRegister);
        }
        /// <summary>
        /// PDFXE에 이벤트 등록 또는 해제
        /// </summary>
